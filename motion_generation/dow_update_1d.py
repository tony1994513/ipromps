...
def main():
    ...
    for idx, t in enumerate(timestamp):
        if t <= dow:
            index = idx 
    dow_t = timestamp[:index+1]
    dow_obs = obs_data_post_arr[:index+1,:]
    ...
    for ipromp in ipromps_set:
        ...
        idx_max = ipromp.estimate_alpha(alpha_temp, dow_obs, dow_t)
    ...
    for task_idx, ipromp in enumerate(ipromps_set):
        for idx in range(len(dow_t)):
            ipromp.add_viapoint(dow_t[idx] / alpha_max_list[task_idx], dow_obs[idx,:])
