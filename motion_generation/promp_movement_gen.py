...
promps_set = joblib.load(os.path.join(datasets_path, 'pkl/promps_set.pkl'))
...
def main():
    ...
    obs_data_post_arr = promps_set[0].min_max_scaler.transform(obs_data)
    ...
    for task_idx, promp in enumerate(promps_set):
    ...
    traj_full = []
    for promp_id, promp in enumerate(promps_set):
    ...
    joblib.dump(promps_set, os.path.join(datasets_path, 'pkl/promps_set_post_offline.pkl'))
    # save the robot traj
    print('Saving the robot traj...')
    joblib.dump([traj_full, obs_data_dict,promp_viapoint], os.path.join(datasets_path, 'pkl/promp_robotTraj_offline.pkl'))
